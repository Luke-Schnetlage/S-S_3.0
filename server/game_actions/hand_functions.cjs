...
async function select_random_card_from_deck(playerID, gameID){
...
async function remove_card_from_deck(card_space) {
  const { data, error } = await supabase
    .from('cards_in_deck')
    .delete(card_space)
  if (error) {
...
/*
async function draw(playerID, gameID) {
card = select_random_card_from_deck(playerID, gameID).then(
